...
export const getReviews = async (req, res) => {
  try {
    const { id } = req.params; // Assuming the productId comes from the URL params

    // Find reviews that match the productId
    const reviews = await Review.find({ productId: id, status: true }); // Only active reviews (status: true)

    if (reviews.length === 0) {
      return res.status(404).json({ message: 'No reviews found for this product.' });
    }

    // Send back the found reviews
