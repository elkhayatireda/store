...
import { ImageUp, X, Image, Trash, Eye, ChevronLeft } from "lucide-react";
...
export default function UpdateReview() {
  ...
  const fetchReviewData = async (id) => {
    try {
      const reviewResponse = await axiosClient.get(`/reviews/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    if (formData.images && formData.images.length > 0) {
      console.log('dfkfkfkf'),
      formDataToSend.append("images", JSON.stringify(formData.images));
  }
    try {
      const response = await axiosClient.post(`/reviews/update/${id}`,formDataToSend);
